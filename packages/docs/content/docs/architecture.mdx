...
Extending Fuse only requires Dart — no Rust, no C. Write a Dart class, add a JSX type declaration, register it. Any Flutter developer already knows enough. And because the Dart side is ordinary Flutter, any [pub.dev](https://pub.dev) package can be wrapped and driven from JSX — see [Using pub.dev packages](/docs/extending/using-packages).
...
| `registerHandle(name, factory)` | Imperative control and navigator pages | `FuseHandle<T>` wrapping a persistent native object (controllers, focus nodes, Flutter `Page` objects via `FusePageHandle`) |
