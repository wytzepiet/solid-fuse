...
`Icon` renders a single glyph from an icon font. It maps to Flutter's `Icon` widget and takes an [`IconData`](#icondata) object describing which glyph to draw.
...
| `color` | `ColorInput` | Icon color (see [ColorInput](/docs/building/ui-and-layout#colorinput)) |
