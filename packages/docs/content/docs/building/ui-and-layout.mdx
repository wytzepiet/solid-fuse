---
title: UI & Layout
description: Build interfaces with views, text, and layout
---
...
|---|---|---|
| `layout` | `LayoutInput` | Layout children (see [Layout](#layout) below) |
...
Every element that accepts children supports `layout` — this is intentional. JSX can't enforce "exactly one child" vs "many children" at the type level, so instead of separate `<row>` and `<column>` components, Fuse uses `layout` as a universal layout config.

```tsx
{/* Horizontal row with spacing */}
<view layout={{ direction: "horizontal", gap: 12 }}>
  <text>Left</text>
  <text>Right</text>
</view>

{/* Centered content filling available space */}
<view layout={{ align: "center", justify: "center", expand: true }}>
  <text>Centered</text>
</view>

{/* Space between items */}
<view layout={{ justify: "spaceBetween" }}>
  <text>Start</text>
  <text>End</text>
</view>
```
...
```tsx
<view layout={{ direction: "horizontal", expand: true }}>
...
`<gestureDetector>` also supports `layout` for arranging children, so you can use it as a tappable container directly.
