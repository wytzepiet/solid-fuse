...
The `child` getter is reactive — it wraps `node.buildLayout()` in a `ListenableBuilder`, so page content updates when signals change without re-triggering the page transition animation.
...
import type { LayoutInput } from "solid-fuse";
...
function SettingsPage() {
  return (
    <cupertinoPage title="Settings" layout={{ gap: 16 }}>
...
- Pages support `layout` via `buildLayout()` — layout props work on page elements just like any other
