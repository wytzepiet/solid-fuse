...
import type { ColorInput, LayoutInput } from "solid-fuse";
...
## The layout + buildLayout() mechanic

> If you add `layout?: LayoutInput` to your JSX type and use `node.buildLayout()` in your Dart widget, **layout just works**. Users of your widget can pass `layout={{ direction: "horizontal", gap: 8 }}` and children will arrange automatically — you don't write any layout code.
  ...
  Widget build(BuildContext context) {
    return Card(
      child: Padding(
        ...
        child: node.buildLayout(),  // ← reads layout prop automatically
      ),
    );
  }
}
...
### How buildLayout() works

- Reads the `layout` prop from the node
- If there's a `layout` config: wraps children in a Flutter `Flex` widget (Row/Column) with the specified gap, alignment, and justification
- If there's only one child and no layout config: returns the child directly (no wrapper)
- This single-child optimization avoids unnecessary layout nodes

### childWidgets vs buildLayout()

- **`node.buildLayout()`** — reads the `layout` prop and applies layout. Use this for anything that should support user-configured layout. This is the recommended default.
